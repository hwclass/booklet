    ...
    const Booklet = function () {
      ...
      */
        this.createView = function (name, options) {
            let defaults = typeof options !== 'undefined' ? options : {};
      ...
      */
        this.getService = function (serviceName) {
            let selectedService;
            for (const servicesCounter = 0, len = selfOfBooklet.SERVICES.length; servicesCounter < len; servicesCounter++) {
      ...
      */
        this.publish = function (topic, info) {
            if (!selfOfBooklet.TOPICS[topic] || !selfOfBooklet.TOPICS[topic].queue.length)
                return;
            const items = selfOfBooklet.TOPICS[topic].queue;
            for (const x = 0; x < items.length; x++) {
                items[x](info || {});
            }
        };
    };
    ...
    const Page = function (options) {
       ...
       */
        this.register = function (moduleName, context) {
            selfOfPage.MODULES.push({
                name: moduleName,
                fn: context(selfOfPage),
                started: false
            });
        };
      ...
      */
        this.detach = function (moduleName) {
            const selectedModuleToDetach = selfOfPage.getModule(moduleName, true);
            try {
      ...
      */
        this.start = function (moduleName) {
            const selectedModuleToStart = selfOfPage.getModule(moduleName);
      ...
      */
        this.startAll = function () {
            for (const modulesCounter = 0, len = selfOfPage.MODULES.length; modulesCounter < len; modulesCounter++) {
      ...
      */
        this.stop = function (moduleName) {
            let selectedModuleToStop = selfOfPage.getModule(moduleName);
      ...
      */
        this.subscribe = function (topic, listener) {
            if (!selfOfPage.TOPICS[topic])
                selfOfPage.TOPICS[topic] = { queue: [] };
            const index = selfOfPage.TOPICS[topic].queue.push(listener);
            return (index => {
                return {
                    remove() {
      ...
      */
        this.getModule = function (moduleName, withIndex) {
            let selectedModule, indexOfTheModule, builtModuleObj;
            for (const modulesCounter = 0, len = selfOfPage.MODULES.length; modulesCounter < len; modulesCounter++) {
      ...
      */
        this.getService = function (serviceName) {
            let selectedService;
            for (const servicesCounter = 0, len = selfOfPage.SERVICES.length; servicesCounter < len; servicesCounter++) {
